...
function timeSpanToString(startDate, endDate) {
  ...
  const minutes = Math.floor((hours > 0 ? (delta % (hours * 60 * 60 * 1000)) : delta)
   / (60 * 1000));
  const seconds = Math.floor((delta - (hours * 60 * 60 * 1000) - (minutes * 60 * 1000)) / 1000);
  const mc = delta - (hours * 60 * 60 * 1000) - (minutes * 60 * 1000) - (seconds * 1000);

  let temp = (hours < 10 ? '0' : '') + hours;
  temp += (minutes < 10 ? ':0' : ':') + minutes;
  temp += (seconds < 10 ? ':0' : ':') + seconds;
  // eslint-disable-next-line no-nested-ternary
  temp += (mc < 10 ? '.00' : (mc < 100 && mc >= 10) ? '.0' : '.') + mc;
